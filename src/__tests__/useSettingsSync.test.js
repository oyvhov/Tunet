...
describe('useSettingsSync', () => {
  beforeEach(() => {
    ...
    deleteSettingsDevice.mockResolvedValue({ success: true, deleted_current: 1, deleted_history: 2 });
    updateSettingsDeviceLabel.mockResolvedValue({ success: true, device_label: 'Living room tablet' });
    ...
    collectSnapshot.mockReturnValue({ version: 1, layout: { pagesConfig: { header: [], pages: ['home'], home: [] } }, appearance: {} });
  ...
  it('loads current settings metadata on init', async () => {
    ...
    const { result } = renderHook(() => useSettingsSync({ haUserId: 'user-1', contextSettersRef: { current: {} } }));
  ...
  it('loads a specific revision from server when provided', async () => {
    ...
    expect(fetchCurrentSettings.mock.calls.some((call) => (
      call[0] === 'user-1' && typeof call[1] === 'string' && call[2] === 2
    ))).toBe(true);
  });
  ...
  it('applies newer server revision automatically on sync tick', async () => {
    ...
    fetchCurrentSettings
      .mockResolvedValueOnce({
        revision: 1,
        updated_at: '2026-02-22T12:00:00.000Z',
        data: { version: 1, layout: { pagesConfig: { header: [], pages: ['home'], home: [] } }, appearance: {} },
      })
      .mockResolvedValueOnce({
        revision: 2,
        updated_at: '2026-02-22T12:05:00.000Z',
        data: { version: 1, layout: { pagesConfig: { header: ['x'], pages: ['home'], home: [] } }, appearance: {} },
      });
    ...
    expect(applySnapshot).toHaveBeenCalledWith(
      { version: 1, layout: { pagesConfig: { header: ['x'], pages: ['home'], home: [] } }, appearance: {} },
      {}
    );
  });
  ...
  it('applies published server config when device had no prior revision', async () => {
    ...
    fetchCurrentSettings
      ...
      .mockResolvedValueOnce({
        ...
        data: { version: 1, layout: { pagesConfig: { header: ['published'], pages: ['home'], home: [] } }, appearance: {} },
      });
    ...
    expect(applySnapshot).toHaveBeenCalledWith(
      { version: 1, layout: { pagesConfig: { header: ['published'], pages: ['home'], home: [] } }, appearance: {} },
      {}
    );
  });
  ...
  it('publishes current config to other devices', async () => {
    ...
    expect(publishCurrentSettings).toHaveBeenCalledWith(expect.objectContaining({
      ha_user_id: 'user-1',
  ...
  it('publishes only selected target devices when provided', async () => {
    ...
    expect(publishCurrentSettings).toHaveBeenNthCalledWith(1, expect.objectContaining({
      ha_user_id: 'user-1',
      source_device_id: expect.any(String),
      target_device_id: 'wall-tablet',
      history_keep_limit: expect.any(Number),
    }));
    expect(publishCurrentSettings).toHaveBeenNthCalledWith(2, expect.objectContaining({
      ha_user_id: 'user-1',
      source_device_id: expect.any(String),
  ...
  it('renames a known device and refreshes list', async () => {
    fetchCurrentDevices.mockResolvedValue([{ device_id: 'tablet-1', device_label: 'Living room tablet', revision: 2, updated_at: '2026-02-22T12:01:00.000Z' }]);
    ...
    expect(updateSettingsDeviceLabel).toHaveBeenCalledWith('user-1', 'tablet-1', 'Living room tablet');
