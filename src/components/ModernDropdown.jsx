...
export default function ModernDropdown({ label, icon: Icon, options, current, onChange, map }) {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between px-6 py-4 rounded-2xl hover:transition-all group border" style={{backgroundColor: 'var(--glass-bg)', borderColor: 'var(--glass-border)'}}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full mt-2 z-50 border rounded-2xl overflow-hidden shadow-2xl" style={{backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)'}}>
