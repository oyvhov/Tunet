  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between px-6 py-4 rounded-2xl group transition-all popup-surface popup-surface-hover">
        <div className="flex items-center gap-3"><Icon className="w-4 h-4 text-gray-400 group-hover:text-[var(--accent-color)] transition-colors" /><span className="text-xs font-bold uppercase tracking-widest italic" style={{color: 'var(--text-secondary)'}}>{String(getLabel(current) || placeholder)}</span></div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full mt-2 z-50 rounded-2xl overflow-hidden shadow-2xl border" style={{backgroundColor: 'var(--modal-bg)', borderColor: 'var(--glass-border)'}}>
          <div className="max-h-48 overflow-y-auto">
            {(options || []).map((option) => (
              <button key={option} onClick={() => { if (typeof onChange === 'function') onChange(option); setIsOpen(false); }} className={`w-full text-left px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all ${current === option ? 'text-[var(--accent-color)]' : 'text-[var(--text-secondary)] hover:bg-[var(--glass-bg-hover)] hover:text-[var(--text-primary)]'}`} style={{backgroundColor: current === option ? 'var(--accent-bg)' : 'transparent'}}>{String(getLabel(option))}</button>
            ))}
          </div>
        </div>
      )}
