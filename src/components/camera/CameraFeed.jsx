...
export function normalizeCameraStreamEngine(value) {
  ...
  if (raw === 'go2rtc' || raw === 'custom' || raw === 'mse') return 'go2rtc';
...
export function getCameraSourcePlan({
  ...
  // Keep configured go2rtc players first. Older Tunet versions treated a saved
  // custom URL as the preferred source in Auto mode, so changing this order
  // silently replaced existing MSE streams with Home Assistant WebRTC.
  if (customPlayerUrl && ['auto', 'go2rtc', 'webrtc'].includes(engine)) {
    plan.push('custom');
  }
  if (['auto', 'go2rtc', 'webrtc', 'ha'].includes(engine) && supportsWebRtc) {
    plan.push('webrtc');
  }
  if (
    hasConnection &&
    (['go2rtc', 'ha', 'webrtc'].includes(engine) || supportsHls || !capabilitiesKnown)
  ) {
    plan.push('hls');
  }
...
export function normalizeGo2rtcMode(value) {
  const raw = String(value || '').toLowerCase();
  if (raw === 'webrtc' || raw === 'mse') return raw;
  return 'auto';
}

export function getGo2rtcModeFromUrl(value) {
  const match = String(value || '').match(/[?&]mode=([^&#]*)/i);
  if (!match) return 'auto';
  try {
    return normalizeGo2rtcMode(decodeURIComponent(match[1]));
  } catch {
    return normalizeGo2rtcMode(match[1]);
  }
}

export function applyGo2rtcMode(value, mode) {
  const url = String(value || '').trim();
  if (!url) return '';

  const normalizedMode = normalizeGo2rtcMode(mode);
  ...
  if (normalizedMode === 'auto') params.delete('mode');
  else params.set('mode', normalizedMode);
...
const CameraImage = memo(
  /** @param {any} props */ function CameraImage({
    src,
    alt,
    fit,
    onReady,
...
const CustomCameraPlayer = memo(
  /** @param {any} props */ function CustomCameraPlayer({ src, title, onReady, onError }) {
    const [loaded, setLoaded] = useState(false);
    return (
      <>
...
const HlsCameraPlayer = memo(
  /** @param {any} props */ function HlsCameraPlayer({
    conn,
    entityId,
    resolveUrl,
    posterUrl,
    ...
    useEffect(() => {
      ...
      timeoutRef.current = window.setTimeout(() => onError('hls-timeout'), SOURCE_TIMEOUT_MS);
...
const WebRtcCameraPlayer = memo(
  /** @param {any} props */ function WebRtcCameraPlayer({
    conn,
    entityId,
    posterUrl,
    fit,
    ...
    useEffect(() => {
      ...
      timeoutRef.current = window.setTimeout(() => onError('webrtc-timeout'), SOURCE_TIMEOUT_MS);
...
const CameraFeed = memo(
  /** @param {any} props */ function CameraFeed({
    entityId,
    entity,
    conn,
    getEntityImageUrl,
    ...
    const customPlayerUrl = useMemo(() => {
      ...
      if (!resolved) return '';
      const configuredMode = settings?.cameraGo2rtcMode;
      const playerUrl = configuredMode ? applyGo2rtcMode(resolved, configuredMode) : resolved;
      return getEntityImageUrl(playerUrl);
    }, [customTemplate, entityId, getEntityImageUrl, settings?.cameraGo2rtcMode]);
